...
from ..towards_realtime_mot import *
from ..datasets.towards_realtime_mot_datasets import JointDataset, collate_fn
from utils import *
...
class TrainModel( Towards_Realtime_MOT ):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if resume:
            ...
            # Set optimizer
            optimizer = torch.optim.SGD(filter(lambda x: x.requires_grad, self.model.parameters()), lr=opt.lr, momentum=.9)
            ...
            if cfg.endswith('yolov3.cfg'):
                load_darknet_weights(self.model, osp.join(weights_from, 'darknet53.conv.74'))
                cutoff = 75
            elif cfg.endswith('yolov3-tiny.cfg'):
                load_darknet_weights(self.model, osp.join(weights_from, 'yolov3-tiny.conv.15'))
                cutoff = 15
        ...
        for epoch in range(epochs):
            ...
            for i, (imgs, targets, _, _, targets_len) in enumerate(dataloader):
                ...
                loss, components = self.model(imgs.cuda(), targets.cuda(), targets_len.cuda())
                ...
                for ii, key in enumerate(self.model.module.loss_names):
